...
class Atomwise(OutputModule):
    ...
    def __init__(self, n_in=128, n_out=1, aggregation_mode='sum', n_layers=2, n_neurons=None,
                 ...
                 train_embeddings=False, bn=False, p=0, uncertainty=False, sdr=False, var_coeff=0.1, num_inducing_points = None, per_atom = False, normalize = False):
        ...
        if isinstance(atomref, str):
            atomref = np.load(atomref)
            if atomref.ndim == 1:
                atomref = atomref[..., np.newaxis]

        if atomref is not None:
            self.atomref = nn.Embedding.from_pretrained(torch.from_numpy(atomref),
...
#            )
        ...
        if aggregation_mode == 'sum':
            ...
            self.atom_pool = schnetpack2.custom.nn.base.Aggregate(axis=1, mean=True)
        elif aggregation_mode == 'minmax':
            self.atom_pool = schnetpack2.custom.nn.base.Aggregate(axis=1, mean=False, minmax=True)
...
#            sigma = self.atom_pool(self.sigma_act(self.sigma_net(inputs))/10, atom_mask)  
        ...
        nat = torch.sum((atomic_numbers != 0), 1, keepdim=True).unsqueeze(-1).type(yi.dtype)
        ...
        #if self.atom_pool.minmax:
        yi = self.standardize(yi)
        if self.atomref is not None:
            y0 = self.atomref(atomic_numbers).type(yi.dtype)
            #if self.per_atom:
            #    y0 = y0 / nat
...
class Energy(Atomwise):
    ...
    def __init__(self, n_in, aggregation_mode='sum', n_layers=2, n_neurons=None,
                 ...
                 return_force=False, mean=None, stddev=None, atomref=None, max_z=100, outnet=None, bn=False, p=0, return_stress=False, return_hessian=False, per_atom=False):
        ...
        self.uncertainty    = uncertainty
        self.per_atom = per_atom
       # self.escale = nn.Parameter(torch.tensor(self.stddev))
        #self.fscale = nn.Parameter(torch.tensor(self.stddev))
...
class MultiOutput(Atomwise):
    ...
    def __init__(self, n_in, n_out, aggregation_mode='sum', n_layers=2, n_neurons=None,
                 ...
                 return_force=False, mean=None, stddev=None, atomref=None, max_z=100, outnet=None, bn=False, p=0, var_coeff=0.1,  return_stress=False, return_hessian=False,repulsive=0.,per_atom=False, normalize = False):
        super(MultiOutput, self).__init__(n_in, n_out, aggregation_mode, n_layers, n_neurons, activation,
                                     return_contributions, return_force, create_graph, mean, stddev,
                                     atomref, 100, outnet, train_embeddings, bn, p, uncertainty, sdr, var_coeff=var_coeff,per_atom=per_atom, normalize = False)
        self.return_stress = return_stress
        self.return_hessian = return_hessian
        self.uncertainty = uncertainty
        self.repulsive=repulsive
        self.per_atom = per_atom
       # self.escale = nn.Parameter(torch.tensor(stddev))
...
#            result["y"]     = result["y"][:,0]
        ...
        if self.requires_dr:
            if self.return_stress:
                forces = -grad(result["y"], inputs[Structure.R],
                           grad_outputs=torch.ones_like(result["y"]),
                           create_graph=self.create_graph,retain_graph=True)[0]
                n_batch = inputs[Structure.R].size()[0]
                idx_m = torch.arange(n_batch, device=inputs[Structure.R].device, dtype=torch.long)[:,
                        None, None]
