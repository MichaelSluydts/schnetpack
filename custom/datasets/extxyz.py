    ...
    with connect(dbpath, use_lock_file=False) as conn:
        with open(xyzpath) as f:
            ...
            for i in range(len(atoms)):
                ...
                r_ij, f_ij = None, None
                if cache:
                    r_ij,f_ij = neighbor_gen(at, distance_expansion=None, cutoff=5.0, n_gaussians=25, trainable_gaussians=False,
                 environment_provider=env, collect_triples=False, pair_provider=None, center_positions=True)
                conn.write(atoms[i], data={ExtXYZ.E: energies[i], ExtXYZ.F: forces[i], ExtXYZ.E + 'peratom' : energiesperatom[i], 'Eform' : eform[i], 'Eformperatom' : eformperatom[i], 'Ehull' : ehull[i], 'Ebin' : ebin[i],  'mean': m , 'r_ij' : r_ij, 'f_ij' : f_ij})
...
class ExtXYZ(AtomsData):
    ...
    def __init__(self, dbpath, xyzpath, subset=None, properties=[], environment_provider=SimpleEnvironmentProvider(), collect_triples = False,
        ...
        super(ExtXYZ, self).__init__(dbpath, subset, properties, environment_provider, collect_triples ,pair_provider, center_positions)


from schnetpack2.custom.data import Structure
from fastai.core import T, to_np,to_gpu
...
def neighbor_gen(at, distance_expansion=None, cutoff=5.0, n_gaussians=25, trainable_gaussians=False,
    ...
    properties[Structure.cell_offset] = T(offsets.astype(np.float32)).unsqueeze(0)
