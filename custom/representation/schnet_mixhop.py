...
class MixHopInteraction(nn.Module):
    """
    ...
    def __init__(self, n_atom_basis, n_spatial_basis, n_filters,
                 normalize_filter=False,cutoff=5.0, cutoff_network=schnetpack2.nn.cutoff.HardCutoff, sdr=False, bn=False, filter_network=None, p=0, var_coeff=0.1, attention=0, order=1):
        super(MixHopInteraction, self).__init__()
            ...
            if not sdr:
                if not bn:
                    self.filter_network = nn.Sequential(
                        schnetpack2.nn.base.Dense(order+1, n_filters,
                    ...
                    self.filter_network = nn.Sequential(
                        schnetpack2.custom.nn.layers.BN_drop_lin(order+1, n_filters,
        ...
        self.cfconv = schnetpack2.custom.nn.cfconv.CFConvHop(n_atom_basis, n_filters, n_atom_basis,
                                                  self.filter_network,
                                                  cutoff_network=self.cutoff_network,
                                                  activation=schnetpack2.nn.activations.shifted_softplus,
                                                  normalize_filter=normalize_filter,
                                                  sdr=sdr, bn=bn,p=p,var_coeff=var_coeff, order = order)
        if bn:
            ...
            self.dense = Linear_sdr(n_filters, n_filters, var_coeff=var_coeff)
    ...
    def forward(self, x, r_ij, neighbors, neighbor_mask, f_ij=None):
        ...
        """
        v = self.cfconv(x, r_ij, neighbors, neighbor_mask, f_ij=None)
        #if self.attention > 0:
            #v = self.gatconv(v,0)
        v = self.dense(v)
        return v
        
class MixHopInteraction_old(nn.Module):
    """
    SchNet interaction block for modeling quantum interactions of atomistic systems.
    ...
    def __init__(self, n_atom_basis, n_spatial_basis, n_filters,
                 normalize_filter=False,cutoff=5.0, cutoff_network=schnetpack2.nn.cutoff.HardCutoff, sdr=False, bn=False, filter_network=None, p=0, var_coeff=0.1, attention=0, order=1):
        super(MixHopInteraction_old, self).__init__()
        ...
        if not filter_network:
            if not sdr:
                if not bn:
                    self.filter_network = nn.Sequential(
                        schnetpack2.nn.base.Dense(len(np.arange(0,5,0.1)), n_filters,
                                                 activation=schnetpack2.nn.activations.shifted_softplus),
                        schnetpack2.nn.base.Dense(n_filters, n_filters,
                                                 activation=schnetpack2.nn.activations.shifted_softplus),
                        schnetpack2.nn.base.Dense(n_filters, n_filters,
        ...
        self.bns = nn.BatchNorm2d(order+1, affine = False)
        self.bn  = nn.BatchNorm1d(n_atom_basis, affine = False)
        #if attention > 0:
            #self.gatconv = GATConv(n_atom_basis, n_atom_basis, heads=1, concat=True, negative_slope=0.2, dropout=p, bias=True)
    ...
    def forward(self, x, r_ij, neighbors, neighbor_mask, f_ij=None):
        ...
        """
#        pdb.set_trace()
        sim_mat = torch.stack([torch.exp(-(r_ij-mu).pow(2)/10) for mu in np.arange(0,5,0.1)], dim=-1)#1/(r_ij+1e-3)
#        sim_mat = torch.einsum('bai,bki->bak', x, x)/(r_ij+0.1)
#        N_atoms = neighbor_mask.sum(-1)
#        N_atoms.clamp_(1,N_atoms.max())
#        sim_mat = r_ij#/N_atoms[:,:,None]#(r_ij/r_ij.sum(-1)[:,:,None])
#        sim_mat[sim_mat!=sim_mat] = 0
        
#        nan_mask = ~(sim_mat!=sim_mat)
#        tmp_sim_mat = torch.zeros_like(sim_mat)
#        tmp_sim_mat[nan_mask] = sim_mat[nan_mask]
#        sim_mat = tmp_sim_mat        
 
#        A =  [sim_mat]
#        for i in range(1, self.order+1):
#            A.append(torch.bmm(A[-1], sim_mat))
        
#        A = torch.cat([a.unsqueeze(-1) for a in A], -1)
#        A = self.bns(A.transpose(1,3)).transpose(1,3)
#        A = self.filter_network(A).squeeze(-1)

        A = sim_mat
        A = self.filter_network(A).squeeze(-1)
        
        x = torch.bmm(A,x)
#        x = torch.einsum('bia,bij->baj', A, x)
        
        v = self.dense(x)
#        v = self.bn(v.transpose(1,2)).transpose(1,2)
        return v
...
class SchNetMixHop(nn.Module):
    ...
    def __init__(self, n_atom_basis=128, n_filters=128, n_interactions=1, cutoff=5.0, n_gaussians=25,
                 ...
                 distance_expansion=None, sc=False, start=1.2, debug=False, filter_network_type="original", n_expansion_coeffs=10, sdr=False, bn=False, p=0, var_coeff=0.1, attention=0, return_stress=False, order = 1):
        super(SchNetMixHop, self).__init__()
        ...
        self.embedding = nn.Embedding(max_z, n_atom_basis, padding_idx=0, max_norm=1)

        # spatial features
        self.distances = schnetpack2.custom.nn.neighbors.AtomDistancesAuto(return_vecs=return_stress)
        ...
        if isinstance(n_interactions,list):
            ...
            for i in n_interactions:
                ...
                temp += i*[MixHopInteraction(n_atom_basis=n_atom_basis,
                                  n_spatial_basis=n_gaussians,
                                  n_filters=n_filters,
                                  normalize_filter=normalize_filter, cutoff=self.cutoff,
                                  cutoff_network = cutoff_network,
                                  sdr=sdr, bn=bn, p=p, var_coeff=var_coeff, filter_network=filter_network, attention=attention, order=order)]
                tempint += i
            ...
            self.interactions = nn.ModuleList([
                                                  MixHopInteraction(n_atom_basis=n_atom_basis,
                                                                    n_spatial_basis=n_gaussians,
                                                                    n_filters=n_filters,
                                                                    normalize_filter=normalize_filter,cutoff=self.cutoff,
                                                                    cutoff_network = cutoff_network,filter_network=filter_network,
                                                                    sdr=sdr, bn=bn, p=p, var_coeff=var_coeff, attention=attention, order=order)
                                              ] * n_interactions)
        else:
            interaction_block_list = []
            for _ in range(n_interactions):
                filter_network, self.skip_distance_expansion = get_filter_network(filter_network_type, n_filters, n_expansion_coeffs, cutoff=cutoff, start = start, trainable=trainable_gaussians)
                interaction_block_list.append(MixHopInteraction(n_atom_basis=n_atom_basis, n_spatial_basis=n_gaussians,
                                  n_filters=n_filters, normalize_filter=normalize_filter,cutoff=self.cutoff,cutoff_network = cutoff_network, sdr=sdr, bn=bn, p=p, var_coeff=var_coeff, filter_network=filter_network, attention=attention, order=order))
