...
import schnetpack2.nn as snn

import schnetpack2.custom.nn.acsf
import schnetpack2.custom.nn.neighbors
from schnetpack2.data import StatisticsAccumulator
...
class SymmetryFunctions(nn.Module):
    ...
    def __init__(self,
        ...
        if self.n_angular > 0:
            ...
            self.angular_filter = snn.GaussianSmearing(start=2.5, stop=self.cutoff_radius - 0.5, n_gaussians=n_angular,
                                                       centered=True)
            self.ADF = schnetpack2.custom.nn.acsf.AngularDistribution(self.angular_filter, self.theta_filter, cutoff_functions=self.cutoff,
        ...
        if self.n_radial > 0:
            ...
            self.RDF = schnetpack2.custom.nn.acsf.RadialDistribution(self.radial_filter, cutoff_function=self.cutoff)
        else:
            self.RDF = None
...
#        nbh_idx_j = inputs[Structure.neighbor_pairs_j]
#        shape = nbh_idx_j.size()
#        s = (k,) + tuple([slice(0, d) for d in shape])
#        neighbor_pairs_mask = nbh_idx_j >= 0
#        neighbor_pairs_mask = inputs[Structure.neighbor_pairs_mask] if Structure.neighbor_pairs_mask in inputs.keys() else torch.ones_like(inputs[Structure.neighbor_pairs_j]).float()
        
#        if cuda: neighbor_pairs_mask.cuda()
        ...
        if self.RDF is not None:
            ...
            # Compute distances
            distances = snn.atom_distances(positions, neighbors, neighbor_mask=neighbor_mask, cell_offsets=cell_offsets)
        ...
        if self.ADF is not None:
            ...
            shape = idx_j.size()
            neighbor_pairs_mask = (idx_j >= 0).float()
            #neighbor_pairs_mask = inputs[Structure.neighbor_pairs_mask] if Structure.neighbor_pairs_mask in inputs.keys() else torch.ones_like(inputs[Structure.neighbor_pairs_j]).float()
#            if cuda: neighbor_pairs_mask.cuda()
            ...
            r_ij, r_ik, r_jk = schnetpack2.custom.nn.neighbors.triple_distances_masked(positions, idx_j, idx_k, neighbor_mask, neighbor_pairs_mask, cell, cell_offset_j, cell_offset_k)
