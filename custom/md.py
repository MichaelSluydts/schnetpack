...
from schnetpack2.atomistic import Energy, ElementalEnergy, AtomisticModel
...
from schnetpack2.representation import SchNet, BehlerSFBlock, StandardizeSF
from schnetpack2.utils import read_from_json
...
class AtomsConverter:
    ...
    def convert_atoms(self, atoms):
        ...
        if self.collect_triples:
            ...
            inputs[Structure.cell_offset_j]    = torch.FloatTensor(offsets_j.astype(np.float32))
            inputs[Structure.cell_offset_k]    = torch.FloatTensor(offsets_k.astype(np.float32))
