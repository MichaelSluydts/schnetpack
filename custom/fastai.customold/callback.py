...
class AverageMetric(Callback):
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        ...
        if isinstance(last_target,dict):
            self.count += last_target[sorted(last_target.keys())[0]].shape[0]
            self.val += last_target[sorted(last_target.keys())[0]].shape[0] * self.func(last_output, last_target).detach().cpu()
        elif not is_listy(last_target):
