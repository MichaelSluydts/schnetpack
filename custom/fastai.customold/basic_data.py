...
class DeviceDataLoader():
    ...
    def create(cls, dataset: Dataset, bs: int = 64, shuffle: bool = False, device: torch.device = defaults.device,
               tfms: Collection[Callable] = tfms, num_workers: int = defaults.cpus, collate_fn: Callable = data_collate,
               **kwargs: Any):
...
class DataBunch():
    ...
    def __init__(self, train_dl: DataLoader, valid_dl: DataLoader, fix_dl: DataLoader = None,
    ...
    def create(cls, train_ds: Dataset, valid_ds: Dataset, test_ds: Optional[Dataset] = None, path: PathOrStr = '.',
               bs: int = 64,
               num_workers: int = defaults.cpus, dl_tfms: Optional[Collection[Callable]] = None,
    ...
    def one_batch(self, ds_type: DatasetType = DatasetType.Train, detach: bool = True, denorm: bool = True,
                  cpu: bool = True) -> Collection[Tensor]:
    ...
    def train_ds(self) -> Dataset:
        return self._grab_dataset(self.train_dl)

    @property
    def valid_ds(self) -> Dataset:
        return self._grab_dataset(self.valid_dl)

    @property
    def single_ds(self) -> Dataset:
        return self._grab_dataset(self.single_dl)

    @property
    def loss_func(self) -> Dataset:
        return getattr(self.train_ds, 'loss_func', F.nll_loss)
    ...
    def sanity_check(self):
        ...
        if not hasattr(self.train_ds, 'items') or len(self.train_ds.items) == 0 or not hasattr(self.train_dl,
                                                                                               'batch_sampler'): return
