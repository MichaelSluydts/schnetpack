...
fastai_types = {
    ...
    OptStrList: 'OptStrList', AffineMatrix: 'AffineMatrix', BoolOrTensor: 'BoolOrTensor',
    FloatOrTensor: 'FloatOrTensor',
...
def to_half(b: Collection[Tensor]) -> Collection[Tensor]:
    ...
    if isinstance(b,dict):
        return {k : to_half(v) for k,v in b.items()}
...
def to_device(b: Tensors, device: torch.device):
    ...
    if isinstance(b,dict):
        b = {k : to_device(v,device) for k,v in b.items()}
        return b, b
...
def split_model(model: nn.Module, splits: Collection[Union[nn.Module, ModuleList]], want_idxs: bool = False):
    ...
    if isinstance(splits[0], nn.Module):
        ...
        res = [nn.Sequential(*s) for s in splits]
...
def calc_loss(y_pred: Tensor, y_true: Tensor, loss_func: LossFunction):
    ...
    if hasattr(loss_func, 'reduction'):
        ...
        return loss_func(y_pred, y_true, reduction='none')
...
def flatten_check(out: Tensor, targ: Tensor) -> Tensor:
    ...
    assert len(out) == len(
        targ), f"Expected output and target to have the same number of elements but got {len(out)} and {len(targ)}."
    return out, targ
