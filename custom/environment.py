...
'''
lib = C.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'cell_list.so'))

atom_list = lib.atom_list
atom_list.argtypes = [np.ctypeslib.ndpointer(dtype = np.float32, flags="F_CONTIGUOUS"), np.ctypeslib.ndpointer(dtype = np.float32, flags="F_CONTIGUOUS"), C.c_float, C.c_int, C.POINTER(C.c_int)]
...
class MaartenEnvironmentProvider:
    def __init__(self,cutoff):
        self.cutoff = cutoff

    def get_environment(self, idx, atoms, grid=None):
        at_coords, at_cell, rcut = atoms.positions,atoms.cell,self.cutoff
        N = C.c_int(len(at_coords))
        max_neighbours = C.c_int(-1)
        at_coords = np.asfortranarray(at_coords, dtype =np.float32)
        at_cell   = np.asfortranarray(at_cell, dtype =np.float32)
        output_ptr = atom_list(at_coords.astype(np.float32), at_cell.astype(np.float32), C.c_float(rcut), N, C.byref(max_neighbours))
...
def collect_atom_triples_offsets(nbh_idx, offsets):
    """
    Collect all valid triples of atoms by rearranging neighbor indices obtained
    from an environment provider.
    Args:
    ...
    # Construct possible permutations
    nbh_idx_j = np.tile(nbh_idx, nneigh)
    nbh_idx_k = np.repeat(nbh_idx, nneigh, axis=1)
    offsets_j = np.tile(offsets, (1,nneigh,1))
    offsets_k = np.repeat(offsets, nneigh,axis=1)
    # Remove same interactions and non unique pairs
    triu_idx_row, triu_idx_col = np.triu_indices(nneigh, k=1)
    triu_idx_flat = triu_idx_row * nneigh + triu_idx_col
    nbh_idx_j = nbh_idx_j[:, triu_idx_flat]
    nbh_idx_k = nbh_idx_k[:, triu_idx_flat]
    offsets_j = offsets_j[:, triu_idx_flat]
    offsets_k = offsets_k[:, triu_idx_flat]
    return nbh_idx_j, nbh_idx_k, offsets_j, offsets_k
