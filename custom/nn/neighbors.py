...
def spikes(x):
    return torch.nn.functional.relu(1/2-(x-1/2).abs()) + torch.nn.functional.relu(1/2-(x+1/2).abs())

def atom_distances_auto_cube(positions, neighbors, cell=None, cell_offsets=None,
                   return_vecs=False,
    ...
    N, A, _, d =  pos_xyz.shape

    # Subtract positions of central atoms to get distance vectors
    dist_vec = pos_xyz - positions[:, :, None, :] + 1e-7
    pos_xyz  = dist_vec#spikes(dist_vec.reshape(N,A*A,d).bmm(torch.inverse(cell))).bmm(cell).reshape(N, A, A, d)
    
    # add cell offset
    if cell is not None:
        B, A, N, D = cell_offsets.size()
        cell_offsets = cell_offsets.contiguous().view(B, A * N, D)
...
class AtomDistances(nn.Module):
    ...
    def __init__(self, return_directions=False, return_vecs=False):
    ...
    def forward(self, positions, neighbors, cell=None, cell_offsets=None,
        ...
        return atom_distances(positions, neighbors, cell, cell_offsets,
                              ...
                              neighbor_mask=neighbor_mask,return_vecs=self.return_vecs)

class AtomDistancesAuto(nn.Module):
    """
    Layer that calculates all pair-wise distances between atoms.
    Use advanced torch indexing to compute differentiable distances
    of every central atom to its relevant neighbors. Indices of the
    ...
    def __init__(self, return_directions=False, return_vecs=False):
        super(AtomDistancesAuto, self).__init__()
        self.return_directions = return_directions
        self.return_vecs = return_vecs
    ...
    def forward(self, positions, neighbors, cell=None, cell_offsets=None,
        ...
        Returns:
            torch.Tensor: Distances of every atoms to its neighbors (B x N_at x N_nbh)
        """
        return atom_distances_auto_cube(positions, neighbors, cell, cell_offsets,
                              return_directions=self.return_directions,
                              neighbor_mask=neighbor_mask,return_vecs=self.return_vecs)

def get_offset_distances(dist_vec, cell=None, cell_offsets=None,neighbor_mask=None):
    # add cell offset
    if cell is not None:
        B, A, N, D = cell_offsets.size()
        cell_offsets = cell_offsets.contiguous().view(B, A * N, D)
...
def triple_distances_masked(positions, neighbors_j, neighbors_k, neighbor_mask, neighbor_pairs_mask, cell, cell_offset_j, cell_offset_k):
    """
    ...
    r_ij = get_offset_distances(R_ij, cell, cell_offset_j, neighbor_pairs_mask)
    r_ik = get_offset_distances(R_ik, cell, cell_offset_k, neighbor_pairs_mask)
    r_jk = get_offset_distances(R_jk, cell, cell_offset_j - cell_offset_k, neighbor_pairs_mask)
