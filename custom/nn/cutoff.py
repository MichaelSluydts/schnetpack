...
__all__ = [
    'CosineCutoff', 'MollifierCutoff', 'PhysNetCutoff'
]
...
def cosine_cutoff(distances, cutoff=5.0):
    ...
    if cutoffs.dtype == torch.float16:
        cutoffs *= (distances < cutoff).half()
    else:
...
class PhysNetCutoff(nn.Module):
    ...
    def forward(self, distances):
        return nn.functional.relu(1-6*(distances/self.cutoff).pow(5)+15*(distances/self.cutoff).pow(4)-10*(distances/self.cutoff).pow(3))
