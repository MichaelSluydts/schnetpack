...
from schnetpack2.nn import shifted_softplus, Dense
from schnetpack2.custom.nn.base import DenseReversed
from schnetpack2.custom.nn.layers import Linear_sdr, BN_drop_lin, Linear_sdr_Reversed
import pdb
...
class MLP(nn.Module):
    ...
    def __init__(self, n_in, n_out, n_hidden=None, n_layers=2, activation=shifted_softplus, sdr=False, bn=False, p=0, var_coeff=0.1):
        ...
        if not sdr:
            if bn:
                layers = [BN_drop_lin(self.n_neurons[i], self.n_neurons[i + 1], bn=True, p=p) for i in range(n_layers - 1)]
            else:
                layers = [Dense(self.n_neurons[i], self.n_neurons[i + 1], activation=activation) for i in range(n_layers - 1)]
            layers.append(Dense(self.n_neurons[-2], self.n_neurons[-1], activation=None))
        else:
            layers = [Linear_sdr(self.n_neurons[i], self.n_neurons[i + 1], activation=activation,p=p, var_coeff=var_coeff) for i in range(n_layers - 1)]
            layers.append(Linear_sdr(self.n_neurons[-2], self.n_neurons[-1], activation=None, bias=False, var_coeff=var_coeff))            
...
class PreActivationResidualNN(nn.Module):
    def __init__(self, n_in, n_out, n_hidden=None, n_layers=2, activation=shifted_softplus, sdr=False, bn=False, p=0, var_coeff=0.1):
        super(PreActivationResidualNN, self).__init__()
        # If no neurons are given, initialize
        if n_hidden is None:
            c_neurons = n_in
            self.n_neurons = []
        ...
        if not sdr:
            if bn:
                ...
                layers = [DenseReversed(self.n_neurons[i], self.n_neurons[i + 1], activation=activation) for i in range(n_layers - 1)]
        else:
            layers = [Linear_sdr_Reversed(self.n_neurons[i], self.n_neurons[i + 1], activation=activation,p=p, var_coeff=var_coeff) for i in range(n_layers - 1)]       
    ...
    def forward(self, inputs):
        ...
        """
        return inputs + self.out_net(inputs)
        
class OutputModuleBlock(nn.Module):
    def __init__(self, n_in, n_out, n_hidden=None, n_layers=2, activation=shifted_softplus, sdr=False, bn=False, p=0, var_coeff=0.1):
        super(OutputModuleBlock, self).__init__()

        self.out_net = PreActivationResidualNN(n_in, n_in, n_hidden=None, n_layers=2, activation=shifted_softplus, sdr=False, bn=False, p=0, var_coeff=0.1)
        
#        self.bn = nn.BatchNorm1d(n_in, affine=True, eps=1e-1)
        
        if sdr:
            self.layer_final = Linear_sdr_Reversed(n_in, n_out, activation=None, bias=False, var_coeff=var_coeff)          
        else:
            self.layer_final = DenseReversed(n_in, n_out, activation=None)
    ...
    def forward(self, inputs):
        ...
        """
        output = self.out_net(inputs)
        
#        output = self.bn(output.transpose(1,2)).transpose(1,2)
        
        return self.layer_final(output)   
