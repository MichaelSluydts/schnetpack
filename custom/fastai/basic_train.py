...
from schnetpack2.custom.fastai.torch_core import *
from schnetpack2.custom.fastai.basic_data import *
...
def get_preds(model:nn.Module, dl:DataLoader, pbar:Optional[PBar]=None, cb_handler:Optional[CallbackHandler]=None,
        activ:nn.Module=None, loss_func:OptLossFunc=None, n_batch:Optional[int]=None, no_grad_val:Optional[bool]=True) -> List[Tensor]:
    "Tuple of predictions and targets, and optional losses (if `loss_func`) using `dl`, max batches `n_batch`."
    res = [torch.cat(o).cpu() for o in
           zip(*validate(model, dl, cb_handler=cb_handler, pbar=pbar, average=False, n_batch=n_batch, no_grad_val=no_grad_val))]
...
def validate(model:nn.Module, dl:DataLoader, loss_func:OptLossFunc=None, cb_handler:Optional[CallbackHandler]=None,
        pbar:Optional[PBar]=None, average=True, n_batch:Optional[int]=None, no_grad_val:Optional[bool]=True)->Iterator[Tuple[Union[Tensor,int],...]]:
    ...
    if no_grad_val:
        with torch.no_grad():
            ...
            for xb,yb in progress_bar(dl, parent=pbar, leave=(pbar is not None)):
                ...
                if isinstance(yb,dict):
                    nums.append(yb[sorted(yb.keys())[0]].shape[0])
                else:
                    if not is_listy(yb): yb = [yb]
        ...
        if cb_handler: cb_handler.set_dl(dl)
        #num_samples = len(dl)
        #val_losses, nums = np.array(num_samples),np.array(num_samples)
        for xb,yb in progress_bar(dl, parent=pbar, leave=(pbar is not None)):
            if cb_handler: xb, yb = cb_handler.on_batch_begin(xb, yb, train=False)
            val_loss = loss_batch(model, xb, yb, loss_func, cb_handler=cb_handler)
            ...
            if n_batch and (len(nums)>=n_batch): break
        #start gpu dump
        #https://docs.fast.ai/dev/gpu.html check dit
        ...
        #torch.cuda.empty_cache()
        nums = np.array(nums, dtype=np.float32)
        if average: return (to_np(torch.stack([x.float() for x in val_losses])) * nums).sum() / nums.sum()
        else:       return val_losses
...
def fit(epochs:int, model:nn.Module, loss_func:LossFunction, opt:optim.Optimizer,
        data:DataBunch, callbacks:Optional[CallbackList]=None, metrics:OptMetrics=None,
         no_grad_val:Optional[bool]=True)->None:
        ...
        for epoch in pbar:
            ...
            if not data.empty_val:
                val_loss = validate(model, data.valid_dl, loss_func=loss_func,
                                       cb_handler=cb_handler, pbar=pbar,no_grad_val=no_grad_val)
            else: val_loss=None
...
class Learner():
    ...
    def fit(self, epochs:int, lr:Union[Floats,slice]=defaults.lr,
            wd:Floats=None, callbacks:Collection[Callback]=None, no_grad_val:Optional[bool]=True)->None:
        ...
        fit(epochs, self.model, self.loss_func, opt=self.opt, data=self.data, metrics=self.metrics,
            callbacks=self.callbacks+callbacks, no_grad_val=no_grad_val)
    ...
    def validate(self, dl=None, callbacks=None, metrics=None, no_grad_val=True):
        ...
        cb_handler.on_epoch_begin()
        val_metrics = validate(self.model, dl, self.loss_func, cb_handler,no_grad_val=True)
