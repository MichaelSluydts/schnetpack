...
def evaluatethings(file, model, env, device,ebins=50,fbins=50):
    ...
    for i in tqdm(range(nstructs)):
        ...
        if "sigma_ensemble" in result: sigmas_ensemble[i] = result['sigma_ensemble'].detach().cpu().float()/nat
...
try:
    if args.num_inducing_points:
        learn.model.load_state_dict(torch.load(os.path.join(args.modelpath,"bulkVtrain3200"+"_induced_"+str(sys.argv[3]+".pth")))['model'])
    else:
        learn.model.load_state_dict(torch.load(os.path.join(args.modelpath,"bulkVtrain3200_"+str(sys.argv[3])+".pth"))['model'])
...
conv = schnetpack2.custom.md.AtomsConverter(device=device, environment_provider=env, collect_triples=False)
...
evaluatethings(vasprun_dir, model, env, device,ebins=50,fbins=50)
#model.eval()
